...
$textsFile  = "$basePath\scripts\overlay-texts-v2.json"

if (-not (Test-Path $tmpDir)) { New-Item -ItemType Directory -Path $tmpDir | Out-Null }

# Alle 6 Blanko-Clips in Rotation
$srcVideos = @(
...
function Render-Video {
    ...
    $outMp4 = Join-Path $tmpDir "aug4_${safe}_out.mp4"
...
Write-Host "=== Selektiver Re-Render: $($posts.Count) Posts ===" -ForegroundColor Yellow
$vidIdx = 0; $musIdx = 3; $ok = 0; $fehler = 0  # musIdx=3 damit andere Tracks als zuvor
...
foreach ($post in $posts) {
    ...
    Write-Host "[$datum] $($post.L1) / $($post.L2)" -ForegroundColor Cyan
    ...
    if ($outFile) {
        ...
        try {
            ...
            Write-Host "  Upload OK" -ForegroundColor Green; $ok++
        } catch {
    ...
    if ($url) {
        $reel  = $augRows | Where-Object { $_.Datum -eq $datum -and $_."Post-Typ" -eq "Reel" -and $_.Status -ne "Gepostet" }
        $story = $augRows | Where-Object { $_.Datum -eq $datum -and $_."Post-Typ" -eq "Story" -and $_.Status -ne "Gepostet" }
...
Write-Host "CSV gespeichert." -ForegroundColor Cyan
