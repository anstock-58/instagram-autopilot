...
function Wait-ForVideoUrl {
    ...
    $maxWait = 60   # 60 x 5 Sekunden = 5 Minuten
    for ($i = 1; $i -le $maxWait; $i++) {
        ...
        try {
            ...
            if ($state -eq "done" -or $state -eq "completed" -or $state -eq "ready") {
                ...
                # Volle Response loggen wenn URL noch fehlt
