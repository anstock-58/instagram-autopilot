...
# Erstellt Slideshow-Video via Blotato Visual Templates API und postet auf Instagram @ki_support.
...
if (-not $csvPath) { Write-Output "FEHLER: Kein KI-Support CSV fuer Monat '$monat' gefunden."; exit 1 }
...
function Create-Slideshow {
    param(
        [string]$imageSource,   # Videoprompt aus CSV (Blotato generiert Bild via AI)
    ...
    $slides = @(
        ...
        @{ imageSource = $imageSource; textOverlay = "Kommentiere KI - ich antworte dir" }
    )
