...
**Die Datei `reference/ki-prozessberatung-kmu-methode.md` ist die Vorlage für jede KI-Prozessberatung bei einem Unternehmen. Sagt Andi „analysiere die Firma X" oder startet ein neues KI-Beratungsprojekt, wird diese Methode automatisch und vollständig angewendet (8 Analyseschritte plus Anhänge A–G: Interview-Leitfaden, Mitarbeiter-Fragebogen, Berichtsvorlage, Checkliste, Handwerksbetriebe-Muster, Datenschutz/EU-AI-Act-Checkliste, Tool-Referenz) — ohne dass er sie erwähnen muss. Bei Kunden aus dem Bauwesen zusätzlich `reference/ki-potenzialanalyse-bauwesen.md` als branchenspezifisches Vorwissen lesen (aktuell Teil 1/4: Bauunternehmen).**
...
## Social Media Automation — Neustart im Kopf

### Architektur

```
contentplan_mai_v2.csv  →  post-trigger.ps1  →  Make.com Webhook  →  fal.ai  →  Instagram
```

- **Contentplan**: `outputs/contentplan_mai_v2.csv` — Quelle der Wahrheit für alle geplanten Posts
- **Trigger-Skript**: `scripts/post-trigger.ps1` — liest CSV, findet heutigen Post, feuert Webhook
- **Windows Task**: `NeustartImKopf_PostTrigger` — täglich 17:55 Uhr, startet das Skript
- **Make.com Szenario**: ID 5522498 ("Social Media Automation Neustart im Kopf – Webhook")
- **Webhook-URL**: siehe `context/secrets.md` (gitignored)
- **Log**: `outputs/post-trigger-log.txt` — jeder Lauf wird protokolliert

### Workflow für neue Posts

Andi gibt mir den Auftrag → ich trage die neue Zeile ins CSV ein → Automation läuft automatisch.
...
Alle Tokens, API-Keys und Webhook-URLs sind in `context/secrets.md` gespeichert (gitignored, nicht auf GitHub).
